...
class ChatVC: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    @IBOutlet weak var messagesTextBox: UITextField!
    ...
    @IBAction func sendMsgPressed(_ sender: Any) {
        if AuthService.instance.isLoggedIn {
            ...
            guard let message = messagesTextBox.text else { return }
            
            SocketService.instance.addMessage(messageBody: message, userId: UserDataService.instance.id, channelId: channelId) { (success) in
                if success {
                    self.messagesTextBox.text = ""
                    self.messagesTextBox.resignFirstResponder()
                    print("messageSent!")
                }
            }
        }
        ...
        MessageService.instance.findAllChannel { (success) in
            if success {
                ...
                if MessageService.instance.channels.count > 0 {
                    ...
                    self.channelNameLbl.text = "Create the first Channel!"
                }
